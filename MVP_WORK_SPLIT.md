...
1. Set up Vite React.
2. Add Tailwind CSS and shadcn/ui as the dashboard component library.
3. Add React Query API hooks.
4. Build the single-screen dashboard.
5. Add ingest trigger/upload flow.
6. Render KPIs, table, filters, run status, and event timeline.
7. Polish the judge-facing demo flow.
8. Keep shadcn usage scoped to UI components; do not change React Query, Zustand, or API boundaries.
...
6. `apps/web/src/components/ui/*` generated by shadcn/ui
7. `apps/web/components.json`
8. Tailwind and Vite alias configuration required by shadcn/ui

### shadcn/ui Setup

Use https://ui.shadcn.com/ for ready-made, accessible dashboard components. This should accelerate UI quality without expanding the MVP into a generic design-system project.

Recommended first components:

1. `button` for ingest actions.
2. `card` for KPI cards and dashboard panels.
3. `badge` for run status, event status, and reorder signals.
4. `select` for store and run filters.
5. `table` for normalized records.
6. `scroll-area` for event history and wide data regions.
7. `skeleton` for loading states.
8. `alert` for API and ingest errors.
9. `separator` for dashboard section boundaries.

Implementation notes:

1. Install shadcn/ui inside `apps/web` only.
2. Keep generated UI components local to the web app.
3. Use React Query for server data, mutations, loading states, errors, and refetching.
4. Use Zustand only for local UI state such as selected store, selected run, filters, or demo mode.
5. Do not add sidebars, auth screens, multi-page navigation, or generic CRUD views before the core demo works.
6. Preserve desktop and mobile usability.
...
7. Loading skeletons while dashboard data is fetching.
8. Clear error alert when the API or ingest request fails.
...
5. shadcn/ui components are used for the main dashboard controls and panels.
6. `npm run typecheck` and `npm run build` pass for `apps/web`.
7. The single dashboard screen remains usable on desktop and mobile.
